...
export default class Docs extends React.Component {
  ...
  componentDidMount() {
    if (window && window.Gumshoe && this.pageNavContainer.current) {
      ...
      let pageTocLinks = this.pageNavContainer.current.getElementsByTagName(
        "a"
      );
          ...
          <div className="docs-content">
                ...
                <div className="post-content" ref={this.pageContent}>
                  ...
                  {docsRootPath !== pageUrl && !_.isEmpty(childPages) && (
                    <ul id="docs-section-items" className="docs-section-items">
                      {_.map(childPages, (childPage, index) =>
                        this.renderChildPage(childPage, index)
                      )}
                    </ul>
                  )}
                </div>
            ...
            <nav id="page-nav" className="page-nav">
              <div id="page-nav-inside" className="page-nav-inside sticky">
                <h2 className="page-nav-title">Перейти к разделу</h2>
                {headerLinks}
              </div>
            </nav>
          </div>
  ...
  return _.reduce(
    ...
    (nodes, childNode) => {
      ...
      if (tagNames.includes(nodeName)) {
        let idAttr = childNode.attrs
          ? _.find(childNode.attrs, { name: "id" })
          : null;
        ...
        if (!idValue) {
          if (!textContent) {
            console.warn(
              "can not create link to element without id and without text content, element"
            );
